...
- **Canonical test render**: `loop/render_tick.sh <N>` renders `examples/space/space-source.jpg` **in-process** via `loop/render.py` (NOT the Flask app — the long-running app never reloads edited engine constants) with baked settings (centered seed, levels 90, smooth 0.0, lum_mix 0.8, method=flow) and writes `loop/output/iter_NNN.{svg,png,stats.json}`. The `stats.json` records the `source` so the scorer can compare against it.
