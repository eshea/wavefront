...
- **Canonical test render**: `loop/render_tick.sh <N>` renders `examples/space/space-source.jpg` **in-process** via `loop/render.py` (NOT the Flask app — the long-running app never reloads edited engine constants) with baked settings (centered seed, levels 60, smooth 0.0, lum_mix 0.8, method=wave) and writes `loop/output/iter_NNN.{svg,png,stats.json}`. The `stats.json` records the `source` so the scorer can compare against it. (levels 60, not 90: keeps the diamonds large enough that the 434px raster doesn't alias into a moiré.)
